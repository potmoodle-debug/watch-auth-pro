/* Watch Auth Pro — daily watch target and pace tracker
   Version 2.84.2 — 24 September 2026
*/
...
(() => {
  ...
  function completeRma() {
    ...
    // Force the visible target card to reflect the committed state immediately.
    // This is deliberately independent of the normal inspection-counter refresh path.
    const visibleCompleted = document.getElementById('daily-target-completed');
    const visibleFill = document.getElementById('daily-target-fill');
    const visibleBreakdown = document.getElementById('daily-target-breakdown');
    if (visibleCompleted) visibleCompleted.textContent = String(state.completed);
    if (visibleFill) visibleFill.style.width = `${Math.min(100, Math.round((state.completed / state.target) * 100))}%`;
    if (visibleBreakdown) {
      visibleBreakdown.textContent = `${Math.max(0, state.completed - state.rmaCompleted)} Authentication · ${state.rmaCompleted} RMA`;
    }
