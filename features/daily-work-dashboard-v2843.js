(() => {
  ...
  function init() {
    ...
    // Same-tab localStorage writes do not fire the storage event. Watch the
    // shared header count as well so authentication and RMA completions update
    // this dashboard immediately.
    const sharedCount = document.getElementById('inspection-count');
    if (sharedCount) {
      const countObserver = new MutationObserver(refresh);
      countObserver.observe(sharedCount, { childList: true, characterData: true, subtree: true });
    }
